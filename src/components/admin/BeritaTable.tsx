...
import { hapusBerita } from '@/actions/berita'
...
export default function BeritaTable({ data }: { data: BeritaRow[] }) {
  const [search, setSearch] = useState('')
  const [perPage, setPerPage] = useState(10)
  const [page, setPage] = useState(1)
  const [sortField, setSortField] = useState<'judul' | 'tanggal' | 'tipe'>('tanggal')
  ...
  function toggleSort(field: 'judul' | 'tanggal' | 'tipe') {
    if (sortField === field) {
  ...
  function SortIcon({ field }: { field: 'judul' | 'tanggal' | 'tipe' }) {
    ...
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
      ...
      {filtered.length === 0 ? (
        <div className="py-20 text-center text-gray-400">
          ...
          <p>{search ? 'Tidak ditemukan hasil untuk pencarian tersebut.' : 'Belum ada berita. Tambahkan sekarang!'}</p>
        </div>
      ) : (
        <>
